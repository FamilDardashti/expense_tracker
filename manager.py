...
def last_history(args , filename='data/expenses.csv') :
    N_rows = args
    with open(filename, 'r', newline='') as f:
        reader = csv.DictReader(f)
        reader = list(reader)
...
def filter_by_date(  start_range,end_range , filename='data/expenses.csv'):
    ...
    with open(filename, 'r', newline='') as f:
        ...
        filtered_rows = [
            ...
            if start_range <= datetime.strptime(row['date'], "%Y-%m-%d").date() <= end_range
            ]
...
def filter_by_category(args, filename='data/expenses.csv') : 
    with open(filename, 'r', newline='') as f:
        reader = csv.DictReader(f)
        filtered_rows = [
            row for row in reader
            if row.get('category') == args
            ]
        filtered_rows.sort(key=lambda row: datetime.strptime(row['date'], "%Y-%m-%d"))
        table = [
        [item['date'], float(item['amount']), item['category'], item['description']]
